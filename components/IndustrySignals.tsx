...
function SignalItem({ quote, attribution, href }: IndustrySignal) {
  return (
    <article className="w-[85vw] max-w-[85vw] shrink-0 py-1 text-[var(--text)] md:inline-flex md:w-fit md:max-w-[70ch]">
      <div className="inline-flex w-full min-w-0 flex-col gap-1 md:w-fit">
        <div className="inline-flex w-full min-w-0 items-baseline gap-3 md:w-fit md:max-w-[70ch]">
          <p className="truncate overflow-hidden text-[15px] leading-5 tracking-[0.01em] text-ellipsis whitespace-nowrap">
            {quote}
          </p>
          <a
            href={href}
            ...
            className="ml-3 shrink-0 font-semibold text-[var(--link)] underline underline-offset-2 focus-visible:ring-2 focus-visible:ring-[var(--accent-strong)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface)] focus-visible:outline-none"
            aria-label={`Open source for ${attribution}`}
          >
            Source
          </a>
        </div>
        <p className="max-w-[70ch] truncate overflow-hidden text-[11px] text-ellipsis whitespace-nowrap text-[var(--muted)] opacity-70">
          {attribution}
        </p>
      </div>
    </article>
...
function ItemSeparator() {
  return (
    <div
      aria-hidden
      className="mx-6 flex shrink-0 items-center justify-center md:mx-8"
    >
...
export default function IndustrySignals() {
  ...
  return (
    <section
      ...
      className="space-y-2"
    >
      <h2 className="px-1 text-[11px] font-semibold tracking-[0.2em] text-[var(--muted)] uppercase">
        INDUSTRY SIGNALS
      </h2>

      <div className="group relative overflow-hidden rounded-2xl border border-[var(--ring)] bg-[var(--surface)] px-3 py-3 transition duration-200 focus-within:-translate-y-1 focus-within:shadow-xl hover:-translate-y-1 hover:shadow-xl">
        <div
          className="edge-fade-mask relative flex touch-pan-x items-center overflow-x-auto overflow-y-hidden py-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        ...
        >
          ...
          <ul className="hidden w-max items-center motion-reduce:flex">
            {signals.map((signal, index) => (
              <li
                key={`reduced-${signal.attribution}`}
                className="flex items-center"
              >
                <SignalItem
                  quote={signal.quote}
