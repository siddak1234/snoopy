...
export function BlockPalette() {
  return (
    <aside className="relative z-10 flex w-28 shrink-0 flex-col border-r border-[var(--ring)]/50 bg-[var(--surface)]/60 px-2.5 py-3 backdrop-blur-sm">
      <h2 className="px-0.5 text-[0.65rem] font-semibold tracking-widest text-[var(--muted)] uppercase">
        Blocks
      </h2>
      <div className="mt-3 flex flex-col gap-2">
        {BLOCK_DEFS.map(({ type, label }) => (
          ...
          >
            ...
            <span className="text-[0.6rem] leading-tight font-medium text-[var(--text)]">
              {label}
            </span>
          </div>
        ))}
